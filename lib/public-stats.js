 ...
 * @param {(minerId: string, cid: string) => (string[] | undefined)} findDealClients
 */
const updateDailyClientRetrievalStats = async (pgClient, committees, findDealClients) => {
  const clientRetrievalStats = new Map()
  /** @type {Map<string, Array<Measurement>>} */
  const clientMeasurements = new Map()
  for (const c of committees) {
    ...
    for (const clientId of clients) {
      if (clientMeasurements.has(clientId)) {
        clientMeasurements.get(clientId).push(...c.measurements)
      } else {
        clientMeasurements.set(clientId, Array.from(c.measurements))
      }
    }
  }

  for (const [clientId, measurements] of clientMeasurements.entries()) {
