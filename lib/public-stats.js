...
export const updatePublicStats = async ({ createPgClient, committees, allMeasurements, findDealClients }) => {
  ...
  try {
    ...
    await updateDailyClientStats(pgClient, committees, findDealClients)
  } finally {
    await pgClient.end()
  }
}
 ...
 * @param {(minerId: string, cid: string) => (string[] | undefined)} findDealClients
 */
const updateDailyClientStats = async (pgClient, committees, findDealClients) => {
  const clientRetrievalStats = new Map()
  /** @type {Map<string, Array<Measurement>>} */
  const clientToMeasurementsMap = new Map()
  for (const c of committees) {
    ...
    clients.forEach(clientId => {
      if (clientToMeasurementsMap.has(clientId)) {
        clientToMeasurementsMap.get(clientId).push(...c.measurements)
      } else {
        clientToMeasurementsMap.set(clientId, Array.from(c.measurements))
      }
    })
  }

  clientToMeasurementsMap.forEach((measurements, clientId) => {
    for (const m of measurements) {
      const retrievalStats = clientRetrievalStats.get(clientId) ?? { total: 0, successful: 0, successfulHttp: 0 }
      ...
      clientRetrievalStats.set(clientId, retrievalStats)
    }
  })
