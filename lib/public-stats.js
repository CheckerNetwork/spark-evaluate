...
const updateDailyMinerDealsChecked = async (pgClient, committees) => {
  ...
  // Use unnest with ORDINALITY to match arrays by position
  await pgClient.query(`
    ...
    SELECT 
      ...
      arrays.payload_cids
    FROM 
      unnest($1::text[]) WITH ORDINALITY AS miners(miner_id, pos),
      unnest($2::text[][]) WITH ORDINALITY AS arrays(payload_cids, pos)
    WHERE miners.pos = arrays.pos
    ON CONFLICT(day, miner_id) DO UPDATE SET
      payload_cids = array(
