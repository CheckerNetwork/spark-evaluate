...
export const updateDailyClientRetrievalStats = async (pgClient, committees, findDealClients) => {
  const clientRetrievalStats = new Map()
  ...
  for (const [clientId, measurements] of measurementsPerClient.entries()) {
    if (!clientRetrievalStats.has(clientId)) {
      clientRetrievalStats.set(clientId, { total: 0, successful: 0, successfulHttp: 0 })
    }
    const retrievalStats = clientRetrievalStats.get(clientId)
  ...
  const flatStats = Array.from(clientRetrievalStats.entries()).flatMap(
