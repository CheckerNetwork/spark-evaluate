...
export const updatePublicStats = async ({ createPgClient, committees, allMeasurements, findDealClients, findDealAllocators }) => {
  ...
  try {
    ...
    await updateDailyNetworkRetrievalStats(pgClient, committees)
  } finally {
    await pgClient.end()
  }
}
 ...
 * @param {number} stats.successful
 */
export const updateDailyNetworkRetrievalStats = async (pgClient, committees) => {
  let total = 0
  let successful = 0
  for (const c of committees) {
    for (const m of c.measurements) {
      if (m.networkRetrievalResult === 'OK') successful++
      total++
    }
  }

  debug('Updating public daily network wide retrieval stats: total += %s successful += %s', total, successful)
  await pgClient.query(`
    INSERT INTO daily_network_retrieval_stats
      (day, total, successful)
    VALUES
      (now(), $1, $2)
    ON CONFLICT(day) DO UPDATE SET
      total = daily_network_retrieval_stats.total + $1,
      successful = daily_network_retrieval_stats.successful + $2
  `, [
    total,
    successful
  ])
