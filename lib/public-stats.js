...
export const updatePublicStats = async ({ createPgClient, committees, allMeasurements, findDealClients, findDealAllocators }) => {
  ...
  try {
    ...
    await updateNetworkRetrievalStats(pgClient, committees)
  } finally {
    await pgClient.end()
  }
}
 ...
 * @param {number} stats.successful
 */
export const updateNetworkRetrievalStats = async (pgClient, committees) => {
  let total = 0
  let successful = 0
  for (const c of committees) {
    for (const m of c.measurements) {
  ...
  debug('Updating public network wide retrieval stats: total += %s successful += %s', total, successful)
  await pgClient.query(`
    INSERT INTO network_retrieval_stats
      (day, total, successful)
    VALUES
      (now(), $1, $2)
    ON CONFLICT(day) DO UPDATE SET
      total = network_retrieval_stats.total + $1,
      successful = network_retrieval_stats.successful + $2
  `, [
    total,
    successful
  ])
