...
export const updateDailyClientRetrievalStats = async (pgClient, committees, findDealClients) => {
  const flatStats = buildPerActorStats(committees, findDealClients, 'client')
  ...
  `, [
    flatStats.map(stat => stat.actorId),
...
export const updateDailyAllocatorRetrievalStats = async (pgClient, committees, findDealAllocators) => {
  const flatStats = buildPerActorStats(committees, findDealAllocators, 'allocator')
...
/**
* @param {Iterable<Committee>} committees
* @param {(minerId: string, cid: string) => (string[] | undefined)} findDealActors
* @param {string} actorName
* @returns {object[]}
*/
function buildPerActorStats (committees, findDealActors, actorName) {
  const retrievalStatsPerActor = new Map()
  /** @type {Map<string, Measurement[]>} */
  const measurementsPerActor = new Map()
  for (const c of committees) {
    const { minerId, cid } = c.retrievalTask
    const actors = findDealActors(minerId, cid)
    if (!actors || !actors.length) {
      console.warn(`Invalid retrieval task (${minerId}, ${cid}): no deal ${actorName}s found. Excluding the task from daily per-${actorName} stats.`)
      Sentry.captureException(new Error(`Invalid retrieval task: no deal ${actorName}s found.`), {
        extra: {
          minerId,
          cid
        }
    ...
    // Each of the actors in the given list has a deal with the miner and payload CID of this committee's retrieval task
    // Each of the measurements in the committee is associated with every single one of these actors and should be accounted for in the stats
    for (const actorId of actors) {
      if (!actorId) continue
      let value = measurementsPerActor.get(actorId)
      if (!value) {
        value = []
        measurementsPerActor.set(actorId, value)
      }
      value.push(...c.measurements)
    }
  }

  // We iterate over all the measurements that are associated with each actor and update the stats accordingly
  for (const [actorId, measurements] of measurementsPerActor.entries()) {
    if (!retrievalStatsPerActor.has(actorId)) {
      retrievalStatsPerActor.set(actorId, { total: 0, successful: 0, successfulHttp: 0 })
    }
    const retrievalStats = retrievalStatsPerActor.get(actorId)
  ...
  // We need to flatten the map of retrieval stats per actor to an array of objects for the sql query
  const flatStats = Array.from(retrievalStatsPerActor.entries()).flatMap(
    ([actorId, retrievalStats]) => ({ actorId, retrievalStats })
  )
  return flatStats
}
