...
export const updateDailyClientRetrievalStats = async (pgClient, committees, findDealClients) => {
  const retrievalStatsPerClient = new Map()
  /** @type {Map<string, Measurement[]>} */
  const measurementsPerClient = new Map()
  for (const c of committees) {
    const { minerId, cid } = c.retrievalTask
    const clients = findDealClients(minerId, cid)
    if (!clients || !clients.length) {
      console.warn(`Invalid retrieval task (${minerId}, ${cid}): no deal clients found. Excluding the task from daily per-client stats.`)
      Sentry.captureException(new Error('Invalid retrieval task: no deal clients found.'), {
        extra: {
          minerId,
          cid
    ...
    // Each of the clients in the given list has a deal with the miner and payload CID of this committee's retrieval task
    // Each of the measurements in the committee is associated with every single one of these clients and should be accounted for in the stats
    for (const clientId of clients) {
      let value = measurementsPerClient.get(clientId)
      if (!value) {
        value = []
        measurementsPerClient.set(clientId, value)
      }
      value.push(...c.measurements)
    }
  }

  // We iterate over all the measurements that are associated with each client and update the stats accordingly
  for (const [clientId, measurements] of measurementsPerClient.entries()) {
    if (!retrievalStatsPerClient.has(clientId)) {
      retrievalStatsPerClient.set(clientId, { total: 0, successful: 0, successfulHttp: 0 })
    }
    const retrievalStats = retrievalStatsPerClient.get(clientId)
  ...
  // We need to flatten the map of retrieval stats per client to an array of objects for the sql query
  const flatStats = Array.from(retrievalStatsPerClient.entries()).flatMap(
    ([clientId, retrievalStats]) => ({ clientId, retrievalStats })
  )
  ...
  `, [
    flatStats.map(stat => stat.clientId),
    flatStats.map(stat => stat.retrievalStats.total),
    flatStats.map(stat => stat.retrievalStats.successful),
    flatStats.map(stat => stat.retrievalStats.successfulHttp)
  ])
}
...
export const updateDailyAllocatorRetrievalStats = async (pgClient, committees, findDealAllocators) => {
  const retrievalStatsPerAllocator = new Map()
  /** @type {Map<string, Measurement[]>} */
  const measurementsPerAllocator = new Map()
  for (const c of committees) {
    const { minerId, cid } = c.retrievalTask
    const allocators = findDealAllocators(minerId, cid)
    if (!allocators || !allocators.length) {
      console.warn(`Invalid retrieval task (${minerId}, ${cid}): no deal allocators found. Excluding the task from daily per-allocator stats.`)
      Sentry.captureException(new Error('Invalid retrieval task: no deal allocators found.'), {
        extra: {
          minerId,
          cid
        }
    ...
    // Each of the allocators in the given list has a deal with the miner and payload CID of this committee's retrieval task
    // Each of the measurements in the committee is associated with every single one of these allocators and should be accounted for in the stats
    for (const allocatorId of allocators) {
      if (!allocatorId) continue
      let value = measurementsPerAllocator.get(allocatorId)
      if (!value) {
        value = []
        measurementsPerAllocator.set(allocatorId, value)
      }
      value.push(...c.measurements)
    }
  }

  // We iterate over all the measurements that are associated with each allocator and update the stats accordingly
  for (const [allocatorId, measurements] of measurementsPerAllocator.entries()) {
    if (!retrievalStatsPerAllocator.has(allocatorId)) {
      retrievalStatsPerAllocator.set(allocatorId, { total: 0, successful: 0, successfulHttp: 0 })
    }
    const retrievalStats = retrievalStatsPerAllocator.get(allocatorId)
    for (const m of measurements) {
      retrievalStats.total++
      if (m.retrievalResult === 'OK') {
        retrievalStats.successful++
  ...
  // We need to flatten the map of retrieval stats per allocator to an array of objects for the sql query
  const flatStats = Array.from(retrievalStatsPerAllocator.entries()).flatMap(
    ([allocatorId, retrievalStats]) => ({ allocatorId, retrievalStats })
  )
  ...
  `, [
    flatStats.map(stat => stat.allocatorId),
    flatStats.map(stat => stat.retrievalStats.total),
    flatStats.map(stat => stat.retrievalStats.successful),
    flatStats.map(stat => stat.retrievalStats.successfulHttp)
  ])
