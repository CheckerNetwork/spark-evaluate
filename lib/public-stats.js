...
export const updateDailyClientRetrievalStats = async (pgClient, committees, findDealClients) => {
  const flatStats = buildPerPartyStats(committees, findDealClients, 'client')
  ...
  `, [
    flatStats.map(stat => stat.partyId),
...
export const updateDailyAllocatorRetrievalStats = async (pgClient, committees, findDealAllocators) => {
  const flatStats = buildPerPartyStats(committees, findDealAllocators, 'allocator')
...
* @param {'client' | 'allocator'} partyName
* @returns {object[]}
*/
function buildPerPartyStats (committees, findDealActors, partyName) {
