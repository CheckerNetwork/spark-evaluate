...
const updateRetrievalTimings = async (pgClient, committees) => {
  ...
  for (const c of committees) {
    ...
    for (const m of c.measurements) {
      ...
      const ttfbMeasurement = m.first_byte_at - m.start_at
      if (isNaN(ttfbMeasurement)) continue
      ttfbMeasurements.push(ttfbMeasurement)
    }
...
export const updateDailyClientRetrievalStats = async (pgClient, committees, findDealClients) => {
  ...
  for (const c of committees) {
    const { minerId, payload_cid: payloadCid } = c.retrievalTask
    const clients = findDealClients(minerId, payloadCid)
    if (!clients || !clients.length) {
      console.warn(`Invalid retrieval task (${minerId}, ${payloadCid}): no deal clients found. Excluding the task from daily per-client stats.`)
      Sentry.captureException(new Error('Invalid retrieval task: no deal clients found.'), {
        extra: {
          minerId,
          payloadCid
    ...
    for (const clientId of clients) {
      if (measurementsPerClient.has(clientId)) {
        measurementsPerClient.get(clientId).push(...c.measurements)
      } else {
        measurementsPerClient.set(clientId, Array.from(c.measurements))
      }
    }
  }
