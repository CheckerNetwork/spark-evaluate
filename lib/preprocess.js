...
export class Measurement {
  ...
  constructor (m, pointerize = (v) => v) {
    ...
    this.alternativeProviderCheck = {
      statusCode: m.alternative_provider_check?.status_code,
      timeout: m.alternative_provider_check?.timeout,
      carTooLarge: m.alternative_provider_check?.car_too_large,
      endAt: parseDateTime(m.alternative_provider_check?.end_at),
      protocol: m.alternative_provider_check?.protocol,
      providerId: pointerize(m.alternative_provider_check?.provider_id)
    }
  }
}
...
export const getAlternativeProviderRetrievalResult = (measurement) => {
  if (measurement.indexer_result !== 'NO_VALID_ADVERTISEMENT' || !measurement.alternative_provider_check?.status_code) {
  ...
  const alternativeProviderMeasurement = {
    indexer_result: 'OK',
    timeout: measurement.alternative_provider_check?.timeout,
    status_code: measurement.alternative_provider_check?.status_code,
    car_too_large: measurement.alternative_provider_check?.car_too_large,
    end_at: measurement.alternative_provider_check?.end_at,
    protocol: measurement.alternative_provider_check?.protocol,
    provider_id: measurement.alternative_provider_check?.provider_id
  }
