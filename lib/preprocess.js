...
export class Measurement {
  ...
  constructor (m, pointerize = (v) => v) {
    ...
    this.networkRetrievalStatusCode = m.network_retrieval_status_code
    this.networkRetrievalTimeout = m.network_retrieval_timeout
    this.networkRetrievalCarTooLarge = m.network_retrieval_car_too_large
    this.networkRetrievalEndAt = m.network_retrieval_end_at
  }
}
...
export const getNetworkRetrievalResult = (measurement) => {
  const providerRetrievalResult = getRetrievalResult(measurement)
  if (providerRetrievalResult !== 'IPNI_NO_VALID_ADVERTISEMENT' || !measurement.network_retrieval_status_code) {
