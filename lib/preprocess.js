...
export class Measurement {
  ...
  constructor (m, pointerize = (v) => v) {
    ...
    this.networkRetrievalResult = pointerize(getNetworkRetrievalResult(m))
    ...
    this.networkRetrieval = {
      statusCode: m.network_retrieval_status_code,
      timeout: m.network_retrieval_timeout,
      carTooLarge: m.network_retrieval_car_too_large,
      endAt: m.network_retrieval_end_at,
      protocol: m.network_retrieval_protocol
    }
  }
}
...
/**
 * Evaluates the network retrieval result.
 *
 * Network retrieval results are evaluated only if the indexer result is `NO_VALID_ADVERTISEMENT`
 * and the network retrieval status code is set.
 *
 * @param {Partial<import('./typings.js').RawMeasurement>} measurement
 * @return {import('./typings.js').RetrievalResult}
 */
export const getNetworkRetrievalResult = (measurement) => {
  if (measurement.indexer_result !== 'NO_VALID_ADVERTISEMENT' || !measurement.network_retrieval_status_code) {
  ...
  const networkMeasurement = {
    indexer_result: 'OK',
    status_code: measurement.network_retrieval_status_code,
    timeout: measurement.network_retrieval_timeout,
    network_retrieval_status_code: measurement.network_retrieval_status_code,
    car_too_large: measurement.network_retrieval_car_too_large,
    end_at: measurement.network_retrieval_end_at,
    protocol: measurement.network_retrieval_protocol
  }

  return getRetrievalResult(networkMeasurement)
}
